<template>
  <div class="main-content">
        ...
        <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
          <el-form-item>
            ...
            >{{ contents.btnAdAllFont == 1?'删除':'' }}</el-button>
                        ...
                        <el-button
              v-if="isAuth('dingdanxinxi','报表') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              type="warning"
              icon="el-icon-s-data"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}</el-button>
            <el-button
              v-if="isAuth('dingdanxinxi','报表') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}<i class="el-icon-s-data el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('dingdanxinxi','报表') && contents.btnAdAllIcon == 0"
              type="warning"
              @click="chartDialog()"
            >{{ contents.btnAdAllFont == 1?'统计报表':'' }}</el-button>
                      </el-form-item>
		     ...
		     <template slot-scope="scope">
              ...
              <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('dingdanxinxi','接单') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="jiedanxinxiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'接单':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','接单') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="jiedanxinxiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'接单':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','接单') && contents.tableBtnIcon == 0" type="success" size="mini" @click="jiedanxinxiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'接单':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
      ...
      width="800">
                                                                              <div id="quyuChart" style="width:100%;height:600px;"></div>
                                                                                                                                    <span slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            jiedanxinxiCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      ...
      this.$nextTick(()=>{
                                                                                                                                             var quyuChart = this.$echarts.init(document.getElementById("quyuChart"),'macarons');
        this.$http({
            url: "group/dingdanxinxi/quyu",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
            let res = data.data;
